...
return function()
	...
	describe("HSV Tests", function()
		...
		test("convert to hsv", function()
			...
			local h, s, v = color.to_hsv(vmath.vector4(1, 1, 1, 1))
			assert(h == 0, "white h, from color to HSV convertion failed")
			assert(s == 0, "white s, from color to HSV convertion failed")
			assert(v == 1, "white v, from color to HSV convertion failed")
		end)
	end)
